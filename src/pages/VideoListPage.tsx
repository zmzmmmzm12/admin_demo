...
function parseVideoSearchParams(searchParams: URLSearchParams): VideoSearchParams {
  const page = Number(searchParams.get('page') ?? '1')
  const pageSize = Number(searchParams.get('pageSize') ?? '10')
  const keyword = searchParams.get('keyword') ?? ''
  const statusRaw = searchParams.get('status') ?? 'all'
  const status: VideoSearchParams['status'] =
    statusRaw === 'ready' || statusRaw === 'encoding' || statusRaw === 'blocked' ? statusRaw : 'all'
...
export function VideoListPage() {
  ...
  useEffect(() => {
    const parsed = parseVideoSearchParams(urlSearchParams)
    const changed =
  ...
  useEffect(() => {
    ...
    const next = buildVideoSearchParams(params).toString()
    if (current !== next) {
      setUrlSearchParams(buildVideoSearchParams(params), { replace: true })
    }
  ...
  return (
    <section>
      <PageHeader title={t('영상 관리')} description={t('영상 목록 조회와 자막 추가/관리 기능을 제공합니다.')} />
          ...
          {selectedIds.length > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-sm text-slate-500 dark:text-slate-300">
                {t("{count}개 선택됨", { count: selectedIds.length })}
              </span>
              <button
                type="button"
        ...
        <TableContainer tableClassName="w-full min-w-[1000px]">
          ...
          <thead className="text-xs text-slate-400 border-b border-slate-100 dark:border-dark-border dark:text-slate-300 text-center">
              <tr>
                ...
                <th className="align-middle px-6 py-3 whitespace-nowrap">{t('썸네일')}</th>
                <th className="align-middle px-6 py-3 whitespace-nowrap text-left">{t('영상')}</th>
                <th className="align-middle px-6 py-3 whitespace-nowrap">{t('카테고리')}</th>
                <th className="align-middle px-6 py-3 whitespace-nowrap">{t('길이')}</th>
                <th className="align-middle px-6 py-3 whitespace-nowrap">{t('조회수')}</th>
          ...
          <tbody>
              ...
              {!videosQuery.isLoading && !videosQuery.isError && list.length === 0 && (
              ...
              {!videosQuery.isLoading &&
                ...
                list.map((video, index) => (
                  ...
                  >
                    ...
                    <td className="align-middle px-6 py-3">
                      <div className="text-center text-sm text-slate-700 whitespace-nowrap dark:text-white">{video.category}</div>
                    </td>
                    <td className="align-middle px-6 py-3">
                      <div className="text-center text-sm text-slate-700 whitespace-nowrap dark:text-white">{video.duration}</div>
                    </td>
                    <td className="align-middle px-6 py-3">
                      <div className="text-center text-sm text-slate-700 whitespace-nowrap dark:text-white">{video.views.toLocaleString(numberLocale)}</div>
                    </td>
                    <td className="align-middle px-6 py-3">
                      <div className="text-center text-sm text-slate-700 whitespace-nowrap dark:text-white">{dayjs(video.updatedAt).format('YYYY.MM.DD HH:mm')}</div>
                    </td>
                    <td className="align-middle px-6 py-3">
                      <div className="flex items-center justify-center">
                        <div className="flex w-full flex-nowrap items-center justify-end gap-1 whitespace-nowrap">
                          <Link to={`/videos/${video.id}`} className="cursor-pointer whitespace-nowrap rounded-md bg-main-color px-3 py-1.5 text-xs text-white">
                            {t('자막 관리')}
                          </Link>
                          <button
                            type="button"
